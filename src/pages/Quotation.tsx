...
import { Calculator, FileText, Send, Edit, Save } from 'lucide-react'
...
const Quotation = () => {
  ...
  const [editedItems, setEditedItems] = useState(items.map(item => ({ ...item })))
      ...
      <div className="space-y-6">
        <div className="card text-center py-12">
          ...
          <p className="text-gray-600 mb-4">
            Your quotation has been generated and sent to our team.
          </p>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">Review and complete the data for your quotation</p>
      </div>
      <div className="card">
        <form className="space-y-6" onSubmit={e => { e.preventDefault(); handleCreateQuotation() }}>
          ...
          <div>
            <div className="flex items-center space-x-3 mb-4">
              ...
              {!editMode && items.length > 0 && (
                <button type="button" className="ml-2 btn-secondary flex items-center px-2 py-1 text-xs" onClick={handleEdit}>
                  <Edit className="h-4 w-4 mr-1" /> Modify
                </button>
              )}
              {editMode && (
                <button type="button" className="ml-2 btn-primary flex items-center px-2 py-1 text-xs" onClick={handleSaveEdit}>
            ...
            {items.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-gray-600 mb-4">No products to quote</p>
                <button
              ...
              <div className="space-y-4">
                {(editMode ? editedItems : items).map((item) => (
                  <div key={item.id} className="flex flex-col md:flex-row md:justify-between md:items-center py-3 border-b border-gray-200 last:border-b-0">
                    <div>
                      ...
                      {item.dimensions && (
                        <p className="text-sm text-gray-600">
                          Dimensions: {item.dimensions.width} × {item.dimensions.height} × {item.dimensions.depth} cm
                        </p>
                      )}
                    </div>
          ...
          <div className="flex justify-end">
            <button
              type="submit"
              disabled={loading || items.length === 0 || editMode}
